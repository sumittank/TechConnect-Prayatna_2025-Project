...
//         // localStorage.setItem('logintype', response.data.userType); // Store user role
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.token) {
        login(response.data.token, response.data.user.role,response.data.user.email); // Update context
