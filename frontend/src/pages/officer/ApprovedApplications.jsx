...
function ApprovedApplications() {
  ...
  useEffect(() => {
    const fetchApprovedApplications = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/approved-applications`);
        ...
        await Promise.all(
          response.data.map(async (app) => {
            try {
              const nocResponse = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/noc/status/${app._id}`);
              if (nocResponse.data.nocUrl) {
                nocData[app._id] = {
                  nocUrl: nocResponse.data.nocUrl,
                  issuedOn: nocResponse.data.issuedOn || null, 
                };
              }
            } catch (error) {
  ...
  const generateNOC = async (appId) => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/noc/generate-noc/${appId}`);
      if (response.data.nocUrl) {
        setNocStatus((prevStatus) => ({
          ...prevStatus,
          [appId]: {
            nocUrl: response.data.nocUrl,
            issuedOn: response.data.issuedOn || new Date().toISOString(),
          },
        }));
      }
    } catch (error) {
